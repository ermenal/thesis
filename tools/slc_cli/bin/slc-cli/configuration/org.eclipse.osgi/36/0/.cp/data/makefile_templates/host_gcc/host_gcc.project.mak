...
BASE_PKG_PATH = {{PKG_PATH | replace('\\', '/') | replace(' ', '\\ ')}}
...
{%- if source.startswith('$(SDK_PATH)') or source.startswith('$(COPIED_SDK_PATH)') or source.startswith('$(PKG_PATH)') %}
{%- set base_path = 'sdk' %}
{%- else %}
...
{%- set dest_path = source_file | replace('$(SDK_PATH)/','') | replace('$(COPIED_SDK_PATH)/','') | replace('$(PKG_PATH)/','') | replace('../', '_/') -%}
