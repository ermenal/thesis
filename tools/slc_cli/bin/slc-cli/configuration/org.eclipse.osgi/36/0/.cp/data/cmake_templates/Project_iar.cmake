...
{%- elif '$(PKG_PATH)' in source %}
    "{{ source | replace('\\', '/') | replace('$(PKG_PATH)','${PKG_PATH}') }}"
...
{%- elif '$(PKG_PATH)' in include %}
    "{{include | replace("-I","",1) | replace('\\', '/') | replace('"','') | replace('$(PKG_PATH)','${PKG_PATH}') }}{# " #}"
...
{%- elif '$(PKG_PATH)' in source %}
   "{{source | replace('\\', '/') | replace('"','') | replace('$(PKG_PATH)','${PKG_PATH}') }}{# " #}"
{%- elif '$(SDK_PATH)' in source %}
...
{%- if flag.startswith("--config") %}
    {{ flag | replace('--config "', '--config ${CMAKE_CURRENT_LIST_DIR}/../') | reverse | replace('"','',1) | reverse }}
{%- elif " " in flag %}
    "SHELL:{{ flag | replace("$(OUTPUT_DIR)","$<TARGET_FILE_DIR:" + PROJECT_NAME + ">") | replace("$(PROJECTNAME)", PROJECT_NAME) }}"
{%- elif "=" in flag and "\"" in flag %}
    {{ flag | replace("$(OUTPUT_DIR)","$<TARGET_FILE_DIR:" + PROJECT_NAME + ">") | replace("$(PROJECTNAME)", PROJECT_NAME) | replace("=", " ") }}
{%- else %}
    {{ flag | replace("$(OUTPUT_DIR)","$<TARGET_FILE_DIR:" + PROJECT_NAME + ">") | replace("$(PROJECTNAME)", PROJECT_NAME) | replace("--map=", "--map ") }}
{%- endif %}
{%- endfor %}
)
