...
./scripts/clean_both.sh
./scripts/build_both.sh

# copy rtz in slot A en B``
rm -rf image_slot_A/ && cp -r rtz/ image_slot_A/ && rm -rf image_slot_B/ && cp -r rtz/ image_slot_B/ 
# copy slot B linkerfiles etc
cp slot_B_files/tz_secure_memory_autogen.h image_slot_B/rtz_s/autogen/tz_secure_memory_autogen.h && cp slot_B_files/slot_B_secure_linkerfile.ld image_slot_B/rtz_s/autogen/linkerfile.ld && cp slot_B_files/slot_B_nonsecure_linkerfile.ld image_slot_B/rtz_ns/autogen/linkerfile.ld
# manueel image files veranderen (juiste addr, juiste prints)
...
# beide images builden
./scripts/create_image_A.sh && ./scripts/create_image_B.sh

#hier pas mergen, bugfix anders oude images in fresh flash
./scripts/merge.sh

# const in header files image A en B
