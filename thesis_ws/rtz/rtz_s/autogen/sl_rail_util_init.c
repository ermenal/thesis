...
// Instance: inst
static sl_rail_handle_t sli_rail_handle_inst = SL_RAIL_EFR32_HANDLE;

#if SL_RAIL_UTIL_INIT_INST_ENABLE
#if     (SL_RAIL_UTIL_INIT_RX_PACKET_QUEUE_INST_ENTRIES > 0)
static sl_rail_packet_queue_entry_t sli_rail_rx_packet_queue_inst[SL_RAIL_UTIL_INIT_RX_PACKET_QUEUE_INST_ENTRIES];
#endif
#if     (SL_RAIL_UTIL_INIT_RX_FIFO_INST_BYTES > 0)
static SL_RAIL_DECLARE_FIFO_BUFFER(sli_rx_fifo_buffer_inst, SL_RAIL_UTIL_INIT_RX_FIFO_INST_BYTES);
#endif
#if     (SL_RAIL_UTIL_INIT_TX_FIFO_INST_BYTES > 0)
static SL_RAIL_DECLARE_FIFO_BUFFER(sli_tx_fifo_buffer_inst, SL_RAIL_UTIL_INIT_TX_FIFO_INST_BYTES);
#endif
...
static void sl_rail_util_init_inst(void)
{
...
#elif (SL_RAIL_UTIL_INIT_RX_PACKET_QUEUE_INST_ENTRIES > 0)
    .rx_packet_queue_entries = SL_RAIL_UTIL_INIT_RX_PACKET_QUEUE_INST_ENTRIES,
    .p_rx_packet_queue = sli_rail_rx_packet_queue_inst,
#else
...
#elif (SL_RAIL_UTIL_INIT_RX_FIFO_INST_BYTES > 0)
    .rx_fifo_bytes = SL_RAIL_UTIL_INIT_RX_FIFO_INST_BYTES,
    .p_rx_fifo_buffer = sli_rx_fifo_buffer_inst,
#else
    // .rx_fifo_bytes = 0U,
    // .p_rx_fifo_buffer = NULL,
#endif
    .tx_fifo_bytes = SL_RAIL_UTIL_INIT_TX_FIFO_INST_BYTES,
    .tx_fifo_init_bytes = 0U,
#if     (SL_RAIL_UTIL_INIT_TX_FIFO_INST_BYTES > 0)
    .p_tx_fifo_buffer = sli_tx_fifo_buffer_inst,
#else
  ...
  status = sl_rail_init(&sli_rail_handle_inst,
                        &rail_init_config,
...
#if SL_RAIL_UTIL_INIT_TX_DATA_FORMAT_INST_ENABLE
  sl_rail_tx_data_config_t tx_data_config = {
    .tx_source = SL_RAIL_UTIL_INIT_TX_DATA_FORMAT_INST_SOURCE,
    .tx_method = SL_RAIL_UTIL_INIT_TX_DATA_FORMAT_INST_MODE,
  };
  status = sl_rail_config_tx_data(sli_rail_handle_inst,
                                  &tx_data_config);
  APP_ASSERT((SL_RAIL_STATUS_NO_ERROR == status),
             "sl_rail_config_tx_data failed, return value: 0x%04" PRIX32,
             status);
#endif // SL_RAIL_UTIL_INIT_TX_DATA_FORMAT_INST_ENABLE
#if SL_RAIL_UTIL_INIT_RX_DATA_FORMAT_INST_ENABLE
  sl_rail_rx_data_config_t rx_data_config = {
    .rx_source = SL_RAIL_UTIL_INIT_RX_DATA_FORMAT_INST_SOURCE,
    .rx_method = SL_RAIL_UTIL_INIT_RX_DATA_FORMAT_INST_MODE,
  };
  status = sl_rail_config_rx_data(sli_rail_handle_inst,
                                  &rx_data_config);
  APP_ASSERT((SL_RAIL_STATUS_NO_ERROR == status),
             "sl_rail_config_rx_data failed, return value: 0x%04" PRIX32,
             status);
#endif // SL_RAIL_UTIL_INIT_RX_DATA_FORMAT_INST_ENABLE
  ...
  if (SL_RAIL_UTIL_INIT_PROTOCOL_INST_DEFAULT
    ...
    channel_config = (const sl_rail_channel_config_t *)channelConfigs[SL_RAIL_UTIL_INIT_PROTOCOL_PROPRIETARY_INST_INDEX];
  ...
  status = sl_rail_config_channels(sli_rail_handle_inst,
                                   channel_config,
  ...
  if (channel_config != NULL) {
    ...
    uint16_t channel = sl_rail_get_first_channel(sli_rail_handle_inst,
                                                 channel_config);
    if (channel != SL_RAIL_CHANNEL_INVALID) {
      status = sl_rail_prepare_channel(sli_rail_handle_inst, channel);
      APP_ASSERT((SL_RAIL_STATUS_NO_ERROR == status),
                 "sl_rail_prepare_channel failed, return value: 0x%04" PRIX32,
                 status);
    }
  }
  status = sl_rail_util_protocol_config(sli_rail_handle_inst,
  ...
  status = sl_rail_config_cal(sli_rail_handle_inst,
                              0U
  ...
  status = sl_rail_config_events(sli_rail_handle_inst,
                                 SL_RAIL_EVENTS_ALL,
  ...
  status = sl_rail_set_tx_transitions(sli_rail_handle_inst,
                                      &tx_transitions);
  APP_ASSERT((SL_RAIL_STATUS_NO_ERROR == status),
             "sl_rail_set_tx_transitions failed, return value: 0x%04" PRIX32,
             status);
  status = sl_rail_set_rx_transitions(sli_rail_handle_inst,
                                      &rx_transitions);
  APP_ASSERT((SL_RAIL_STATUS_NO_ERROR == status),
